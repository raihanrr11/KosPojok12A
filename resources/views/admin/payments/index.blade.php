            ...
            <div class="bg-white rounded-xl shadow-md p-4">
                <form action="{{ route('admin.payments') }}" method="GET" class="flex flex-col md:flex-row gap-4 items-end">
                    ...
                    <div class="flex gap-2 w-full md:w-auto">
                        <button type="submit"
                            class="flex-1 md:flex-none bg-green-600 text-white px-6 py-2 rounded-lg font-bold text-sm hover:bg-green-700 transition shadow-lg shadow-green-200">
                            Cari
                        </button>
                        <a href="{{ route('admin.payments', ['status' => request('status')]) }}"
                            class="flex-1 md:flex-none bg-gray-100 text-gray-600 px-6 py-2 rounded-lg font-bold text-sm hover:bg-gray-200 transition text-center flex items-center justify-center">
                            Reset
                        </a>
                    </div>
                </form>
            ...
            <div class="bg-white rounded-xl shadow-md overflow-hidden">
                <nav class="flex space-x-2 p-2">
                    <a href="{{ route('admin.payments', ['month' => date('n'), 'year' => date('Y')]) }}"
                        class="flex-1 px-4 py-3 rounded-lg font-bold text-sm text-center transition-all duration-300 {{ (request('month') == date('n') && request('year') == date('Y') && request('status') == null) ? 'bg-gradient-to-r from-indigo-600 to-purple-600 text-white shadow-lg' : 'text-gray-600 hover:bg-gray-100' }}">Bulan
                        Ini</a>
                    ...
                    <a href="{{ route('admin.payments') }}?status=unpaid"
                        class="flex-1 px-4 py-3 rounded-lg font-bold text-sm text-center transition-all duration-300 {{ request('status') == 'unpaid' ? 'bg-gradient-to-r from-gray-500 to-slate-500 text-white shadow-lg' : 'text-gray-600 hover:bg-gray-100' }}">Belum
                        Bayar</a>
                </nav>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
                <div class="divide-y divide-gray-100">
                    @forelse($payments as $payment)
                        ...
                        <form id="verify-form-{{ $payment->id }}" method="POST"
                            action="{{ route('admin.payments.verify', $payment) }}" class="hidden">
                            @csrf @method('PATCH')
                            <input type="hidden" name="status" value="verified">
                        </form>
                        <form id="reject-form-{{ $payment->id }}" method="POST"
                            action="{{ route('admin.payments.verify', $payment) }}" class="hidden">
                            @csrf @method('PATCH')
                                    ...
                                    <div class="flex items-start space-x-4 flex-1">
                                        <div class="flex-shrink-0">
                                            @if($payment->user->photo)
                                                <img src="{{ Storage::url($payment->user->photo) }}"
