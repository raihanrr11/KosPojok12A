            ...
            <div class="flex items-center justify-between">
                ...
                <div class="flex space-x-3">
                    <a href="{{ route('admin.rooms.index') }}"
                        class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-xl shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">
                        Kembali
                    </a>
                    <a href="{{ route('admin.rooms.edit', $room) }}"
                        class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 border border-transparent rounded-xl shadow-sm text-sm font-bold text-white hover:from-indigo-700 hover:to-purple-700 transition-colors">
                        Edit Kamar
                    </a>
                    <form id="delete-room-form" action="{{ route('admin.rooms.destroy', $room) }}" method="POST"
                        class="hidden">
                        @csrf
                        @method('DELETE')
                    </form>
                ...
                <div class="bg-white rounded-2xl shadow-md overflow-hidden">
                    ...
                    @if(count($photoUrls) > 0)
                        ...
                        <div x-data="{ 
                            ...
                            <div
                                class="relative aspect-[4/3] w-full rounded-xl overflow-hidden mb-4 bg-gray-100 shadow-inner group">
                                ...
                                <template x-if="photos.length > 1">
                                    <div>
                                        <button @click="prev()"
                                            class="absolute left-3 top-1/2 -translate-y-1/2 bg-white/80 text-gray-800 hover:text-indigo-600 rounded-full p-2 hover:bg-white shadow-md transition-all focus:outline-none opacity-0 group-hover:opacity-100">
                                        ...
                                        <button @click="next()"
                                            class="absolute right-3 top-1/2 -translate-y-1/2 bg-white/80 text-gray-800 hover:text-indigo-600 rounded-full p-2 hover:bg-white shadow-md transition-all focus:outline-none opacity-0 group-hover:opacity-100">
                                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M9 5l7 7-7 7" />
                                            </svg>
                ...
                <div class="bg-white rounded-2xl shadow-md overflow-hidden relative">
                    ...
                    <div class="p-6">
                        ...
                        <div class="border-t border-gray-100 pt-4 mt-4">
                            <div class="text-sm font-bold text-gray-500 uppercase tracking-wider mb-2">Status Kamar
                            </div>
                            <span
                                class="inline-flex items-center px-4 py-2 rounded-full text-sm font-bold shadow-sm bg-{{ $room->status_color }}-100 text-{{ $room->status_color }}-800">
                                {{ $room->status_label }}
                            </span>
                        </div>
                    </div>
                ...
                <div class="relative bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden"
                    ...
                    <div class="p-6">
                        <p class="text-sm text-gray-600 text-center font-medium mb-6">
                            Semua foto dan data terkait kamar <strong>{{ $room->room_number }}</strong> akan terhapus
                            secara
                            permanen.
                        </p>
                        <div class="flex gap-3">
                            ...
                            <button type="button" @click="document.getElementById('delete-room-form').submit()"
                                class="flex-1 px-4 py-3 rounded-xl text-sm font-black text-white bg-gradient-to-r from-red-500 to-pink-600 hover:from-red-600 hover:to-pink-700 shadow-lg shadow-red-500/30 transition transform hover:scale-[1.02]">Ya,
                                Hapus</button>
                        </div>
                    </div>
                </div>
