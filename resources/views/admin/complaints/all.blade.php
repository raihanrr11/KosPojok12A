...
@section('content')
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
            <form action="{{ route('admin.complaints.all') }}" method="GET"
                        ...
                        @forelse($complaints as $complaint)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-bold text-gray-900">{{ Str::limit($complaint->subject, 30) }}
                                    </div>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <div class="text-xs text-gray-500">Kamar {{ $complaint->user->room_number ?? '-' }}
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    @elseif($complaint->priority === 'medium')
                                        <span
                                            class="px-2 py-1 text-xs font-medium bg-yellow-50 text-yellow-700 rounded-md">Sedang</span>
                                    @else
                                        <span
                                            class="px-2 py-1 text-xs font-medium bg-gray-50 text-gray-700 rounded-md">Rendah</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    @elseif($complaint->status === 'in_progress')
                                        <span
                                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">Diproses</span>
                                    @elseif($complaint->status === 'closed')
                                        <span
                                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800">Ditutup</span>
                                    @else
                                        <span
                                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">Kendala</span>
                                    @endif
                                </td>
