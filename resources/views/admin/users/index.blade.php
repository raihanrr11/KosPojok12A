...
@section('content')
    <div
        ...
        <div class="w-full space-y-6" x-data="{ deleteModal: false, deleteTarget: { name: '', formId: '' } }">
            ...
            <div
                class="w-full relative overflow-hidden rounded-2xl bg-gradient-to-r from-indigo-600 to-purple-600 p-8 shadow-xl">
            ...
            <div class="w-full bg-white rounded-xl shadow-md overflow-hidden">
                <nav class="flex flex-col sm:flex-row gap-2 p-2">
                    <a href="{{ route('admin.users') }}"
                        class="flex-1 px-4 py-3 rounded-lg font-bold text-sm text-center transition-all duration-300 {{ request('status') == null ? 'bg-gradient-to-r from-indigo-600 to-purple-600 text-white shadow-lg' : 'text-gray-600 hover:bg-gray-100' }}">Penghuni
                        Aktif</a>
                    <a href="{{ route('admin.users') }}?status=deleted"
                        class="flex-1 px-4 py-3 rounded-lg font-bold text-sm text-center transition-all duration-300 {{ request('status') == 'deleted' ? 'bg-gradient-to-r from-red-500 to-pink-500 text-white shadow-lg' : 'text-gray-600 hover:bg-gray-100' }}">Pindah
                        / Dihapus</a>
                </nav>
            </div>
            ...
            <div class="w-full grid grid-cols-1 gap-6">
                @forelse($users as $user)
                    <div
                        ...
                        <a href="{{ route('admin.users.show', $user->id) }}" class="absolute inset-0 z-0"
            ...
            <template x-teleport="body">
                <div x-show="deleteModal" x-cloak class="fixed inset-0 z-[9999] flex items-center justify-center p-4"
                    ...
                    <div class="relative bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden"
                        ...
                        <div class="p-6">
                            <p class="text-sm text-gray-600 text-center font-medium mb-2">Anda akan menghapus data penghuni:
                            </p>
