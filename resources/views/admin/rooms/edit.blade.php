...
<div class="space-y-6" x-data="{ deleteModal: false, deleteTarget: { roomId: '', index: '' } }">
    <div class="flex items-center justify-between">
        <h2 class="text-2xl font-bold text-gray-900 text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-purple-600">Edit Kamar {{ $room->room_number }}</h2>
        <a href="{{ route('admin.rooms.index') }}" class="text-sm font-bold text-indigo-600 hover:text-indigo-900 transition-colors">
            ...
            <form action="{{ route('admin.rooms.update', $room) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative group">
                            <span class="absolute left-4 top-3.5 text-gray-400 group-focus-within:text-indigo-600 transition-colors">Rp</span>
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        ...
                        <p class="text-xs text-gray-500 mt-2 ml-1 italic">Dapat memilih lebih dari satu foto. Format: JPG, PNG, JPEG. Max: 2MB per file.</p>
                    </div>
        ...
        <div class="relative bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden"
            ...
            <div class="p-6">
                ...
                <div class="flex gap-3">
                    <button @click="deleteModal = false" class="flex-1 px-4 py-3 border border-gray-200 rounded-xl text-sm font-bold text-gray-600 bg-white hover:bg-gray-50 transition">Batal</button>
                    <button @click="
                        ...
                        form.submit();"
                        class="flex-1 px-4 py-3 rounded-xl text-sm font-black text-white bg-gradient-to-r from-red-500 to-pink-600 shadow-lg shadow-red-500/30 transition transform hover:scale-[1.02]">Ya, Hapus</button>
                </div>
            </div>
        </div>
    </div>
