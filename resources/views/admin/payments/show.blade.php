...
@section('content')
    <div
        ...
        <div class="max-w-4xl mx-auto space-y-6"
            x-data="{ verifyModal: false, rejectModal: false, rejectReason: '', rejectError: false }">
                ...
                <div class="px-6 py-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                    ...
                    <div>
                        @if($payment->status === 'verified')
                            <span
                                class="px-4 py-1.5 rounded-full text-xs font-black bg-green-100 text-green-800 uppercase tracking-wider">✓
                                Terverifikasi</span>
                        @elseif($payment->status === 'rejected')
                            <span
                                class="px-4 py-1.5 rounded-full text-xs font-black bg-red-100 text-red-800 uppercase tracking-wider">✗
                                Ditolak</span>
                        @else
                            <span
                                class="px-4 py-1.5 rounded-full text-xs font-black bg-yellow-100 text-yellow-800 uppercase tracking-wider animate-pulse">⏱
                                Pending</span>
                        @endif
                    </div>
                </div>
                ...
                <div class="p-6 sm:p-8">
                    <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-6">
                        ...
                        <div class="space-y-1">
                            ...
                            <dd class="text-xl font-black text-gray-900">Rp
                                {{ number_format($payment->amount, 0, ',', '.') }}
                            </dd>
                        </div>
                        <div class="space-y-1">
                            <dt class="text-xs font-black text-gray-400 uppercase tracking-widest">Tanggal Pembayaran</dt>
                            <dd class="text-base font-medium text-gray-900">{{ $payment->payment_date->format('d F Y') }}
                            </dd>
                        </div>
                        <div class="space-y-1">
                            ...
                            <dd>
                                <span
                                    class="px-3 py-1 rounded-lg text-xs font-bold bg-blue-50 text-blue-700 border border-blue-100 uppercase">{{ $payment->payment_method }}</span>
                            </dd>
                        </div>
                        ...
                        @if($payment->status === 'rejected' && $payment->admin_notes)
                            <div class="sm:col-span-2 space-y-1 pt-4">
                                <dt class="text-xs font-black text-red-400 uppercase tracking-widest">Alasan Penolakan Admin
                                </dt>
                                <dd
                                    class="bg-red-50 border border-red-100 rounded-2xl p-4 text-sm text-red-800 font-medium italic">
                                    "{{ $payment->admin_notes }}"
                                </dd>
                            </div>
                        @endif
                        ...
                        <div class="sm:col-span-2 pt-6 border-t border-gray-50">
                            <dt class="text-xs font-black text-gray-400 uppercase tracking-widest mb-4">Bukti Pembayaran
                            </dt>
                            <dd>
                                    ...
                                    @if(Str::endsWith($payment->proof_file, ['.jpg', '.jpeg', '.png', '.gif']))
                                        <div class="relative group max-w-lg">
                                            ...
                                            <a href="{{ Storage::url($payment->proof_file) }}" target="_blank"
                                                ...
                                                <span
                                                    class="bg-white px-6 py-3 rounded-2xl font-black text-gray-900 shadow-xl transform translate-y-4 group-hover:translate-y-0 transition-transform">
                                                    Lihat Layar Penuh
                                                </span>
                                            </a>
                                        </div>
                ...
                <div class="bg-white p-8 rounded-3xl shadow-xl border border-gray-100">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <button type="button" @click="verifyModal = true"
                            ...
                            <svg class="w-6 h-6 mr-2 group-hover:rotate-12 transition-transform" fill="none"
            ...
            <template x-teleport="body">
                <div x-show="verifyModal" x-cloak class="fixed inset-0 z-[9999] flex items-center justify-center p-4"
                    ...
                    <div class="relative bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden"
                        ...
                        <div class="p-6">
                            <p class="text-sm text-gray-600 text-center mb-6">Pastikan bukti pembayaran sudah sesuai dengan
                                tagihan.</p>
                            ...
                            <form action="{{ route('admin.payments.verify', $payment) }}" method="POST">
                                ...
                                <div class="mb-4">
                                    <label for="admin_notes"
                                        class="block text-sm font-bold text-gray-700 mb-1.5 ml-1">Alasan
                                        Penolakan <span class="text-red-500">*</span></label>
                                    <textarea name="admin_notes" id="admin_notes" x-model="rejectReason"
                                        @input="rejectError = false" rows="4" required
                                ...
                                <div class="flex gap-3">
                                    <button type="button" @click="rejectModal = false; rejectError = false"
                                        class="flex-1 px-4 py-3 border border-gray-200 rounded-xl text-sm font-bold text-gray-600 bg-white hover:bg-gray-50 transition">Batal</button>
                                    <button type="submit"
                                        @click="if(!rejectReason.trim()){ $event.preventDefault(); rejectError = true; }"
