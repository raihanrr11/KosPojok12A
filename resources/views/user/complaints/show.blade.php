                ...
                <div class="relative z-10 sm:flex sm:items-start sm:justify-between">
                    <div>
                        <div class="flex items-center gap-2 mb-2">
                            ...
                            @if($complaint->status === 'resolved')
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-green-400/30 text-green-100 border border-green-300/40">✓
                                    Selesai</span>
                            @elseif($complaint->status === 'in_progress')
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-yellow-400/30 text-yellow-100 border border-yellow-300/40">⏳
                                    Diproses</span>
                            @elseif($complaint->status === 'closed')
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-gray-400/30 text-gray-100 border border-gray-300/40">✕
                                    Ditutup</span>
                            @else
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-white/20 text-white border border-white/30">●
                                    Menunggu</span>
                            @endif
                        </div>
                        <h1 class="text-2xl font-bold text-white leading-tight">{{ $complaint->subject }}</h1>
                        <p class="mt-1 text-red-100 text-sm">{{ $complaint->category_label }} &mdash;
                    ...
                    <div class="mt-4 sm:mt-0 flex flex-wrap gap-2">
                        @if($complaint->is_public)
                            <span
                                class="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-bold bg-green-900/40 text-green-100 border border-green-400/40">🌐
                                Publik</span>
                        @else
                            <span
                                class="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-bold bg-white/10 text-white border border-white/20">🔒
                                Pribadi</span>
                        @endif

                        @if($complaint->priority === 'high')
                            <span
                                class="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-bold bg-red-900/50 text-red-100 border border-red-400/40">↑
                                Prioritas Tinggi</span>
                        @elseif($complaint->priority === 'medium')
                            <span
                                class="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-bold bg-yellow-900/40 text-yellow-100 border border-yellow-400/40">Prioritas
                                Sedang</span>
                        @else
                            <span
                                class="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-bold bg-white/10 text-white border border-white/20">Prioritas
                                Rendah</span>
                        @endif
                    </div>
                </div>
                ...
                <div class="divide-y divide-gray-100">
                    ...
                    @if($complaint->admin_response)
                        <div class="p-6">
                            ...
                            <div
                                class="bg-gradient-to-br from-blue-50 to-indigo-50 p-4 rounded-xl border-l-4 border-indigo-500">
                        ...
                        <li>
                            <div class="relative pb-8">
                                ...
                                <div class="relative flex space-x-3">
                                    <span
                                        class="h-8 w-8 rounded-full bg-gradient-to-br from-blue-500 to-indigo-500 flex items-center justify-center ring-4 ring-white shadow-sm">
                        ...
                        @if(in_array($complaint->status, ['in_progress', 'resolved', 'closed']))
                            <li>
                                <div class="relative pb-8">
                                    ...
                                    <div class="relative flex space-x-3">
                                        <span
                                            class="h-8 w-8 rounded-full bg-gradient-to-br from-yellow-500 to-orange-500 flex items-center justify-center ring-4 ring-white shadow-sm">
                                            <svg class="h-4 w-4 text-white" fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd"
                        ...
                        @if($complaint->status === 'resolved')
                            <li>
                                <div class="relative">
                                    <div class="relative flex space-x-3">
                                        <span
                                            class="h-8 w-8 rounded-full bg-gradient-to-br from-green-500 to-emerald-500 flex items-center justify-center ring-4 ring-white shadow-sm">
                                            <svg class="h-4 w-4 text-white" fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd"
                        ...
                        @elseif($complaint->status === 'closed')
                            <li>
                                <div class="relative">
                                    <div class="relative flex space-x-3">
                                        <span
                                            class="h-8 w-8 rounded-full bg-gradient-to-br from-gray-400 to-gray-500 flex items-center justify-center ring-4 ring-white shadow-sm">
                                            <svg class="h-4 w-4 text-white" fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd"
                ...
                <div class="bg-gradient-to-r from-red-50 to-pink-50 border border-red-200 rounded-xl p-5">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-sm text-red-700">Keluhan Anda sudah diterima dan akan segera ditanggapi oleh
                                admin.
                            </p>
                        </div>
                    </div>
